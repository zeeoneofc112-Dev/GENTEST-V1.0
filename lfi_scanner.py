...
class LfiScanner(ScannerBase):
    def __init__(self):
        super().__init__()
        self.module_name = "lfi_scanner"
        self.vulnerabilities = []
        self.payloads = load_payloads("lfi.txt")
    
    def scan(self, target):
        """Main scanning function"""
        self.log(f"Starting Local File Inclusion Scanner scan on {target}", "INFO")
        
        # Run all scan methods
        for method_name in ['test_lfi', 'path_traversal', 'wrapper_abuse']:
...
def main():
    ...
    print(f"[36m\n[+] Starting Local File Inclusion Scanner[0m")
