...
class RceScanner(ScannerBase):
    def __init__(self):
        super().__init__()
        self.module_name = "rce_scanner"
        self.vulnerabilities = []
        self.payloads = load_payloads("rce.txt")
    
    def scan(self, target):
        """Main scanning function"""
        self.log(f"Starting Remote Code Execution Scanner scan on {target}", "INFO")
        
        # Run all scan methods
        for method_name in ['scan_rce', 'test_deserialization', 'check_file_uploads']:
...
def main():
    ...
    print(f"[36m\n[+] Starting Remote Code Execution Scanner[0m")
